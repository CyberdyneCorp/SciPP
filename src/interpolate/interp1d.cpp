...
Interp1d::Interp1d(const ndarray& x, const ndarray& y, std::string kind,
  ...
  if (x_.size() != y_.size() || x_.size() < 2)
    throw scipp::value_error("Interp1d: x and y must have equal length >= 2");
}
...
double Interp1d::operator()(double xq) const {
  ...
  if (xq < x_[0] || xq > x_[n - 1]) {
    ...
    throw scipp::value_error("Interp1d: query out of bounds");
  }
  ...
  throw scipp::value_error("Interp1d: unknown kind " + kind_);
}
