...
`scypp::special` SHALL provide `airy` (Ai, Ai', Bi, Bi') and `airye` (exponentially
...
`scypp::special` SHALL provide the complete and incomplete elliptic integrals
...
`scypp::special` SHALL provide `erfcx`, `dawsn`, `wofz`/`voigt_profile`, and
...
`scypp::special` SHALL provide spherical Bessel functions `spherical_jn`/`yn`/`in`/`kn`,
...
`scypp::special` SHALL provide `sici` (Si, Ci) and `shichi` (Shi, Chi), matching
...
`scypp::special` SHALL provide `lambertw`, `zeta`/`zetac`, `struve`/`modstruve`,
...
`scypp::special` SHALL provide the confluent and Gauss hypergeometric functions
