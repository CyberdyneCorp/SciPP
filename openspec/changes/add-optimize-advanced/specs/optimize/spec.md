...
`scipp::optimize` SHALL provide `linprog` (linear programming) and `milp`
...
`scipp::optimize::minimize` SHALL provide the methods `Powell`, `CG`, `Newton-CG`,
...
`scipp::optimize` SHALL provide `differential_evolution`, `basinhopping`,
...
`scipp::optimize` SHALL provide `nnls` and `lsq_linear`, matching SciPy. (oracle: scipy/optimize/_nnls.py, _lsq)
