...
## 1. cluster.vq
- [x] `include/scypp/cluster/cluster.hpp`; `src/cluster/*.cpp` in build; export from umbrella
...
## 3. io
- [x] `include/scypp/io/io.hpp`; `src/io/io.cpp` in build; export
- [x] Matrix Market `mmread`/`mmwrite`
- [x] WAV `wavread`/`wavwrite` (16/32-bit PCM)
- [x] ARFF `loadarff` (numeric)

## 4. Oracle + validation
- [x] Generator writes golden `sample.mtx`/`sample.wav`/`sample.arff` + expected values; `SCYPP_GOLDEN_DIR` define
...
- [x] Check off Phase 12 in `bootstrap-scypp-foundation/tasks.md`; update README to v1.0
