...
ScyPP already ships the two geometry primitives it needs — a KD-tree and a 2-D
Delaunay triangulation — so `nearest` and `linear` griddata need no NumPP changes.

## What changes

Extends the **interpolate** capability — `scypp::interpolate`, validated against
the SciPy oracle:
...
- Affected code: new `src/interpolate/griddata.cpp`, header decl in
  `include/scypp/interpolate/interpolate.hpp`, `tests/test_interpolate_griddata.cpp`,
  extended oracle generator. Reuses `scypp::spatial` KDTree and Delaunay.
