# Bootstrap ScyPP foundation

## Why

ScyPP is a clean-room C++20 port of SciPy — the SciPy sibling of NumPP (NumPy)
...
1. **Built on NumPP.** SciPy is built on NumPy; ScyPP is built on NumPP. NumPP's
   `ndarray`, dtype system, broadcasting, ufuncs, linalg, fft and random are the
   array engine. ScyPP does not re-implement arrays — it ports SciPy's
...
- **scypp-foundation** — project skeleton, the NumPP dependency contract (ScyPP
  ...
  (`scypp::error` hierarchy mirroring SciPy/NumPP error categories).
- **backend-acceleration** — how ScyPP-specific kernels (FFT, GEMM-backed
...
- **No** new GPU device-management layer — ScyPP reuses NumPP's. This change
