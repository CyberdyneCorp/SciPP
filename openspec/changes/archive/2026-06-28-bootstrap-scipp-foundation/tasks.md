# Tasks — Bootstrap SciPP foundation

## Phase 0 — Foundation (this change)

### Project skeleton & build
- [x] CMake ≥ 3.25 / C++20 project; `include/scipp/` + `src/<subpackage>/` layout with reserved subpackage dirs
- [x] Generated `config.hpp` with `SCIPP_WITH_{BLAS,LAPACK,CUDA,OPENCL,METAL}` flags (default OFF)
- [x] NumPP wired as a **pinned Conan/vcpkg release dependency** resolved via `find_package` (not vendored), umbrella `scipp/scipp.hpp`, `version.hpp.in`
...
- [x] GPU flag propagation: `SCIPP_WITH_<GPU>` enables the matching `NUMPP_WITH_<GPU>`
- [x] CPU-only build verified green (all backend flags OFF); iOS/Android cross-compile contract documented

### NumPP integration
- [x] Confirm public-API contract: SciPP routines take/return `numpp::ndarray`, no SciPP array type
- [x] `scipp::error` aligned with `numpp::error`; `value_error`/`linalg_error`/`not_implemented_error`; status-struct convention for SciPy result objects
