...
SciPP SHALL maintain a prioritized, tiered backlog of SciPy subpackages not yet
...
SciPP SHALL port `scipy.special` (gamma/`gammaln`/`beta`/`digamma`, error functions
...
- WHEN the SciPP special function is evaluated
...
SciPP SHALL port `scipy.linalg`: basic ops (`inv`/`det`/`solve`/`lstsq`/`pinv`/
...
- GIVEN a matrix `A`
- WHEN a SciPP decomposition (`lu`/`qr`/`svd`/`cholesky`) is computed
...
SciPP SHALL port `scipy.fft`/`scipy.fftpack`: complex and real transforms
...
- WHEN SciPP computes `fft`/`rfft`/`dct`/`dst` (and inverses)
- THEN the results are `allclose` to SciPy and round-trip to the input within
  tolerance

### Requirement: optimize parity (Phase 4)

SciPP SHALL port `scipy.optimize`: scalar/multivariate minimization (`minimize` with
...
- WHEN a SciPP method minimizes it
...
SciPP SHALL port `scipy.integrate` (`quad`/`dblquad`/`simpson`/`trapezoid`,
...
SciPP SHALL port `scipy.interpolate`: 1-D interpolation (`interp1d`,
...
- WHEN a SciPP interpolant is evaluated at the query points
- THEN the values are `allclose` to SciPy's and reproduce the samples at the knots

### Requirement: stats parity (Phase 7)

SciPP SHALL port `scipy.stats`: continuous and discrete distributions (pdf/cdf/ppf/
...
- WHEN SciPP evaluates pdf/cdf/ppf and runs a hypothesis test
...
SciPP SHALL port `scipy.signal`: convolution/correlation (`convolve`/`fftconvolve`/
...
- WHEN SciPP filters the signal and estimates its spectrum
- THEN the filtered output and spectral estimates are `allclose` to SciPy's

### Requirement: sparse parity (Phase 9)

SciPP SHALL port `scipy.sparse`: sparse array/matrix formats (CSR/CSC/COO/DIA/LIL/
...
- WHEN SciPP computes SpMV and a sparse solve
...
SciPP SHALL port `scipy.spatial`: `KDTree`/`cKDTree` nearest-neighbor queries,
...
- GIVEN a point set
- WHEN SciPP builds a KD-tree, queries neighbors, and computes pairwise distances
- THEN neighbor indices and distance matrices match SciPy's (distances `allclose`)

### Requirement: ndimage parity (Phase 11)

SciPP SHALL port `scipy.ndimage`: filters (`gaussian_filter`/`uniform_filter`/
...
- WHEN SciPP applies a filter, a morphology op, and a measurement
- THEN the outputs are `allclose` (or label-equivalent) to SciPy's

### Requirement: cluster, io, and datasets parity (Phase 12)

SciPP SHALL port `scipy.cluster` (`kmeans`/`kmeans2`/`vq`, hierarchical `linkage`/
...
- GIVEN sample data
- WHEN SciPP runs k-means/linkage and reads/writes a supported file format
